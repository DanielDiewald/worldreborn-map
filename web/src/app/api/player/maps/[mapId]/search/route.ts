...
export async function GET(request:Request,{params}:{params:Promise<{mapId:string}>}){
  ...
  for(const feature of features){if(!feature.label)continue;if(matches(feature.label,term)||matches(feature.short_description,term))add({kind:"feature",id:Number(feature.feature_id),name:feature.label,subtitle:feature.short_description,featureId:Number(feature.feature_id),markerId:null,href:null});}
