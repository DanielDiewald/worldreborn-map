...
const ALLOWED_TYPES = new Set<SearchEntityType>(["person","group","location"]);
...
export async function GET(request:Request,{params}:{params:Promise<{projectId:string}>}){
  ...
  const types=(requested.length?requested:["person","group","location"]).filter((value):value is SearchEntityType=>ALLOWED_TYPES.has(value as SearchEntityType));
