...
import { createMapMarker, listMapMarkers } from "@/lib/maps";
...
export async function GET(_request: Request, { params }: { params: Promise<{ projectId: string; mapId: string }> }) {
  ...
  try {
    return NextResponse.json({ markers: await listMapMarkers(projectId, mapId) });
  } catch {
...
export async function POST(request: Request, { params }: { params: Promise<{ projectId: string; mapId: string }> }) {
  ...
  try {
    const markerId = await createMapMarker(projectId, mapId, await request.json());
    return NextResponse.json({ markerId }, { status: 201 });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Marker could not be created" }, { status: 400 });
  }
}
