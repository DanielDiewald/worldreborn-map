...
import { listRaceRegistry, type RaceRegistryRow } from "@/lib/race-registry";
...
function SubspeciesRow({ projectId, race }: { projectId: number; race: RaceRegistryRow }) {
  ...
  return <div className={styles.subspeciesRow}>
    ...
    <div className={styles.subspeciesActions}>{originHref ? <Link className="button ghost" href={originHref}>⌖ Karte</Link> : null}<Link className="button ghost" href={`/admin/projects/${projectId}/races/${race.raceId}/image`}>Bild zuschneiden</Link><Link className="button ghost" href={`/admin/projects/${projectId}/races/${race.raceId}`}>Öffnen</Link></div>
  </div>;
}

function SpeciesCard({ projectId, race, children }: { projectId: number; race: RaceRegistryRow; children: RaceRegistryRow[] }) {
  const originHref = mapHref(projectId, race);
  return <article className={styles.speciesCard}>
    <div className={styles.speciesHeader}>
      ...
      <div className={styles.cardActions}>{originHref ? <Link className="button ghost" href={originHref}>⌖ Auf Karte</Link> : null}<Link className="button ghost" href={`/admin/projects/${projectId}/races/${race.raceId}/image`}>Bild zuschneiden</Link><Link className="button" href={`/admin/projects/${projectId}/races/${race.raceId}`}>Codex öffnen</Link></div>
    </div>
...
export default async function RacesPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  const [project, races, mapsRaw] = await Promise.all([getProject(projectId), listRaceRegistry(projectId), listProjectMaps(projectId)]);
  if (!project) notFound();
  ...
  const byParent = new Map<number, RaceRegistryRow[]>();
  ...
  return <AdminShell projectId={projectId} projectName={project.name} section="races" eyebrow={`${project.name} / Welt`} title="Spezies & Subspezies">
    ...
    {unknownRace ? <section className={styles.unknownBar}><div>{preview(unknownRace, true)}<span><strong>Unbekannt</strong><small>System-Fallback für Characters, deren Spezies noch nicht feststeht.</small></span></div><div className="row"><Link className="button ghost" href={`/admin/projects/${projectId}/races/${unknownRace.raceId}/image`}>Bild zuschneiden</Link><Link className="button ghost" href={`/admin/projects/${projectId}/races/${unknownRace.raceId}`}>Fallback öffnen</Link></div></section> : null}

    <section className={styles.registrySection}>
      <div className={styles.registryHeading}><div><span className="panel-kicker">TAXONOMISCHER CODEX</span><h2>{rootSpecies.length} Haupt-Spezies · {subspecies.length} Subspezies</h2><p>Counts werden pro Registry-Lauf aggregiert und verwaltete Bilder als kleine Avatar-Derivate ausgeliefert.</p></div><RaceCreateDialog projectId={projectId} rootSpecies={rootSpecies} maps={maps}/></div>
