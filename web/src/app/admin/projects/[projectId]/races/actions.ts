...
export async function createRaceAction(projectId: number, formData: FormData) { await requireAdminSession(); await assertProject(projectId); const name = text(formData, "name"); const source = await resolveEntityImageSource(projectId, formData, { title: name || "Spezies" }); const created = await createRace(projectId, raceInput(formData, source.image, source.mediaId)); redirect(`/admin/projects/${projectId}/races/${created.raceId}`); }
export async function updateRaceAction(projectId: number, raceId: number, formData: FormData) { await requireAdminSession(); await assertProject(projectId); const current = await getRace(projectId, raceId); if (!current) throw new Error("Spezies wurde nicht gefunden."); const source = await resolveEntityImageSource(projectId, formData, { current: current.image, title: current.name }); const imageMediaId = source.uploaded ? source.mediaId : source.removed || source.image !== current.image ? null : current.imageMediaId; await updateRace(projectId, raceId, raceInput(formData, source.image, imageMediaId)); refreshRace(projectId,raceId); revalidatePath(`/admin/projects/${projectId}/npcs`); revalidatePath(`/admin/projects/${projectId}/characters`); }
