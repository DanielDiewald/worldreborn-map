...
import { listFamilyTreePeople, listFamilyTreesPaginated } from "@/lib/entities/family-trees";
import { parsePagination } from "@/lib/pagination";
import { getProject } from "@/lib/projects";
import { createFamilyTreeAction } from "./actions";
import styles from "./family-trees.module.css";

type Search=Promise<{page?:string;pageSize?:string}>;
...
export default async function FamilyTreesPage({params,searchParams}:{params:Promise<{projectId:string}>;searchParams:Search}){
  ...
  const pagination=parsePagination(search);const [project,trees,people]=await Promise.all([getProject(projectId),listFamilyTreesPaginated(projectId,pagination),listFamilyTreePeople(projectId)]);if(!project)notFound();
  ...
  return <AdminShell projectId={projectId} projectName={project.name} section="family-trees" eyebrow={`${project.name} / Wissen`} title="Stammbäume">
    <div className="page-heading compact-heading"><div><div className="breadcrumb"><Link href={`/admin/projects/${projectId}`}>{project.name}</Link><span>/</span><strong>Stammbäume</strong></div><h1>Familienlinien & Dynastien</h1><p>Benannte Stammbäume organisieren denselben kanonischen Familiengraphen. Personen bleiben immer <code>person + n_id</code>; Verwandtschaft wird nicht doppelt gespeichert.</p></div></div>
    <section className={`panel-card ${styles.createPanel}`}><details><summary className="button primary">＋ Neuen Stammbaum anlegen</summary><form action={createFamilyTreeAction.bind(null,projectId)} className="stack" style={{marginTop:16}}><div className="field-grid two"><label>Name<input name="name" placeholder="z. B. Haus Valen"/></label><label>Untertitel<input name="subtitle" placeholder="z. B. Königliche Linie"/></label><label>Root-Person<select name="rootPersonId" defaultValue=""><option value="">Ohne feste Root-Person</option>{people.map((p)=><option key={p.person_id} value={p.person_id}>{p.name} · {p.kind}</option>)}</select></label><label>Sichtbarkeit<select name="visibilityMode" defaultValue="admin_only"><option value="admin_only">Nur Admin</option><option value="all_players">Alle Spieler</option><option value="selected_players">Ausgewählte Spieler</option></select></label></div><label>Beschreibung<textarea name="description" placeholder="Worum geht es in diesem Familienzweig?"/></label><div className="row end"><button className="primary">Stammbaum erstellen</button></div></form></details></section>
    ...
    <Pagination pathname={path} searchParams={{}} page={trees.page} pageSize={trees.pageSize} total={trees.total} totalPages={trees.totalPages}/>
  </AdminShell>;
}
