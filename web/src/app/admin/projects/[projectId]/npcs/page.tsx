...
import { listEntityImageProfiles } from "@/lib/entity-image-profiles";
...
export default async function NpcListPage({params,searchParams}:{params:Promise<{projectId:string}>;searchParams:Search}){
  ...
  if(!project)notFound();
  const imageProfiles=await listEntityImageProfiles(projectId,"person",result.items.map((npc)=>npc.nId));
