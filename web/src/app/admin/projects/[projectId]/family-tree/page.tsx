...
export default async function FamilyTreePage({params}:{params:Promise<{projectId:string}>}){
 ...
 return <AdminShell projectId={projectId} projectName={project.name} section="family-trees" eyebrow={`${project.name} / Wissen`} title="Family Tree"><div className="page-heading compact-heading"><div><div className="breadcrumb"><Link href={`/admin/projects/${projectId}/relationships`}>Beziehungen</Link><span>/</span><strong>Family Tree</strong></div><h1>Stammbaum</h1><p>NPCs, Götter und Spielercharaktere teilen denselben Personenbaum.</p></div></div><section className="panel-card family-tree"><ul>{roots.map((root)=>render(root.n_id,new Set()))}</ul>{roots.length===0?<div className="empty-state"><strong>Keine Wurzel gefunden</strong><span>Der Baum kann zyklische oder noch unvollständige Parent-Relationen enthalten.</span></div>:null}</section></AdminShell>;
}
