...
export default async function RelationshipsPage({params}:{params:Promise<{projectId:string}>}){
  ...
  return <AdminShell projectId={projectId} projectName={project.name} section="relationships" eyebrow={`${project.name} / Wissen`} title="Beziehungen">
    ...
    <section className="panel-card"><div className="table-meta"><span><strong>{relationships.length}</strong> Beziehungen</span><span><Link href={`/admin/projects/${projectId}/family-trees`}>Stammbäume</Link> · <Link href={`/admin/projects/${projectId}/relationship-graph`}>Graph</Link></span></div>
