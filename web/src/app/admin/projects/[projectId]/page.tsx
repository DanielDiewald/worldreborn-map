...
export default async function ProjectDashboardPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  return <AdminShell projectId={projectId} projectName={project.name} section="dashboard" eyebrow={`${project.name} / Übersicht`} title="Dashboard">
    ...
    <div className="dashboard-grid"><section className="panel-card recent-panel"><div className="panel-heading"><div><span className="panel-kicker">WORLD DATA</span><h2>Zuletzt bearbeitete NPCs</h2></div><Link href={`/admin/projects/${projectId}/npcs`}>Alle NPCs →</Link></div>{dashboard.recentNpcs.length?<div className="recent-list">{dashboard.recentNpcs.map(npc=><Link key={npc.id} href={`/admin/projects/${projectId}/npcs/${npc.id}`} className="recent-row"><div className="entity-avatar small">{npc.image&&npc.image!=="noimage"?<img src={npc.image} alt=""/>:npc.name.slice(0,1).toUpperCase()}</div><div className="recent-main"><strong>{npc.name}</strong><span>{npc.title||"NPC"}</span></div><time>{formatDate(npc.updatedAt)}</time><span className="row-arrow">→</span></Link>)}</div>:<div className="empty-state"><strong>Noch keine NPCs</strong><span>Erstelle den ersten Eintrag für diese Welt.</span></div>}</section>
