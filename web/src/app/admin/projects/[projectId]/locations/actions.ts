...
function optionalNumber(formData:FormData,name:string){const value=String(formData.get(name)??"").trim();return value?Number(value):null;}
function optionalString(formData:FormData,name:string){const value=String(formData.get(name)??"").trim();return value||null;}
function values(formData:FormData,coatOfArm:string){return{name:formData.get("name"),locationType:formData.get("locationType")||undefined,locationKind:formData.get("locationKind")||"other",slug:optionalString(formData,"slug"),description:formData.get("description")||undefined,coatOfArm:coatOfArm==="noimage"?undefined:coatOfArm,parentLocId:optionalNumber(formData,"parentLocId"),ownerNpcId:optionalNumber(formData,"ownerNpcId"),capitalLocId:optionalNumber(formData,"capitalLocId"),population:optionalNumber(formData,"population"),mapId:optionalNumber(formData,"mapId"),mapFeatureId:optionalNumber(formData,"mapFeatureId"),visibilityMode:formData.get("visibilityMode")||"admin_only"};}
