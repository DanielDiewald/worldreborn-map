...
export default async function LocationDetailPage({params}:{params:Promise<{projectId:string;locationId:string}>}){
  ...
  const [project,location,locations,npcs]=await Promise.all([getProject(projectId),getLocation(projectId,locationId),listLocations(projectId),listNpcs(projectId)]);if(!project||!location)notFound();
  return <AdminShell projectId={projectId} projectName={project.name} section="locations" title={String(location.name)} eyebrow={`${project.name} / Locations`}>
    ...
    <section className="entity-hero"><div className="entity-avatar hero-avatar">{location.coat_of_arm?<img src={String(location.coat_of_arm)} alt=""/>:String(location.name).slice(0,1).toUpperCase()}</div><div className="entity-hero-main"><span className="page-kicker">LOCATION #{locationId}</span><h1>{String(location.name)}</h1><p>{location.location_type?String(location.location_type):"Location"}</p></div></section>
    ...
    <section className="danger-zone"><div><h2>Location archivieren</h2><p>Der Datensatz wird nicht hart gelöscht.</p></div><form action={archiveLocationAction.bind(null,projectId,locationId)}><details><summary className="button danger">Archivieren…</summary><button className="danger">Archivierung bestätigen</button></details></form></section>
  </AdminShell>;
}
