...
export async function getLocation(projectId: number, locationId: number) {
  const result=await pool.query(`SELECT * FROM locations WHERE camp_id=$1 AND loc_id=$2 AND archived_at IS NULL`,[projectId,locationId]);return result.rows[0]??null;
}

export async function createLocation(projectId: number, input: unknown) {
  const data=locationSchema.parse(input);await validateReferences(projectId,data.parentLocId,data.ownerNpcId);const result=await pool.query<{loc_id:number}>(`INSERT INTO locations(camp_id,name,coat_of_arm,parent_loc_id,location_type,description,owner_n_id,population,visibility_mode,metadata,updated_at) SELECT c.camp_id,$2,$3,$4,$5,$6,$7,$8,$9,'{}'::jsonb,now() FROM campaigns c WHERE c.camp_id=$1 AND c.status<>'archived' RETURNING loc_id`,[projectId,data.name,data.coatOfArm||null,data.parentLocId||null,data.locationType||null,data.description||null,data.ownerNpcId||null,data.population??null,data.visibilityMode]);if(result.rowCount!==1)throw new Error("Project not found or archived.");return result.rows[0].loc_id;
}
