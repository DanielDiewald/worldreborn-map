...
export async function listGroups(projectId: number) {
  const result = await pool.query(
    `SELECT g.gr_id, g.name, g.image, g.notes, g.motto, g.loc_id,
            g.members AS estimated_member_count,
            g.group_type, g.visibility_mode, l.name AS location_name,
            (SELECT count(*)::int FROM group_memberships gm
              WHERE gm.project_id=g.camp_id AND gm.group_id=g.gr_id) AS known_members
...
export async function getGroup(projectId: number, groupId: number) {
  const result = await pool.query(
    `SELECT g.*, g.members AS estimated_member_count,
            (SELECT count(*)::int FROM group_memberships gm WHERE gm.project_id=g.camp_id AND gm.group_id=g.gr_id) AS known_members,
...
export async function setGroupMembership(args: { projectId:number; groupId:number; entityType:string; entityId:number; role?:string }) {
  ...
  const ref = await resolveEntityReference({ projectId: args.projectId, entityType: args.entityType, entityId: args.entityId });
  await pool.query(
    ...
    [args.projectId,args.groupId,ref.type,ref.id,args.role || null]);
  await pool.query(
    `INSERT INTO audit_log(project_id,actor_type,action,entity_type,entity_id,metadata)
     VALUES($1,'admin','membership.created',$2,$3,$4::jsonb)`,
    [args.projectId,ref.type,ref.id,JSON.stringify({group_id:args.groupId,role:args.role || null})]);
}
...
export async function removeGroupMembership(args:{projectId:number;groupId:number;entityType:string;entityId:number}) {
  ...
  await pool.query(`DELETE FROM group_memberships WHERE project_id=$1 AND group_id=$2 AND entity_type=$3 AND entity_id=$4`,
    [args.projectId,args.groupId,ref.type,ref.id]);
  await pool.query(
    `INSERT INTO audit_log(project_id,actor_type,action,entity_type,entity_id,metadata)
     VALUES($1,'admin','membership.removed',$2,$3,$4::jsonb)`,
    [args.projectId,ref.type,ref.id,JSON.stringify({group_id:args.groupId})]);
}
