...
export async function listCharacters(projectId:number){const r=await pool.query<{char_id:number;n_id:number;name:string;image:string;race:string;alive:boolean;class:string;age:number;location_name:string;player_id:number|null;player_name:string|null;visibility_mode:string}>(
`SELECT c.char_id,n.n_id,n.name,n.image,c.race,c.alive,c.class,c.age,l.name AS location_name,
  ...
  WHERE n.camp_id=$1 AND n.archived_at IS NULL ORDER BY n.name,c.char_id`,[projectId]);return r.rows;}
...
`SELECT c.*,n.name,n.gender,n.image,n.notes,n.public_description,n.admin_notes,n.visibility_mode,
        u.user_id AS player_id,COALESCE(u.display_name,u.name) AS player_name
   FROM charakters c JOIN npcs n ON n.n_id=c.n_id
