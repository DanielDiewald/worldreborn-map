...
export type FamilyTreeEdge={a:number;b:number;code:string;label:string;inverseLabel:string|null;directed:boolean;category:string;source:string};
...
export async function getFamilyTreeGraph(projectId:number,treeRef:number|"all"){
  ...
  const relationshipSql=named?`SELECT r.entity_a_id::int AS a,r.entity_b_id::int AS b,rt.code,rt.label,rt.inverse_label AS "inverseLabel",rt.directed,rt.category,'relationship'::text AS source FROM relationships r JOIN relationship_types rt ON rt.relationship_type_id=r.relationship_type_id JOIN family_tree_members ma ON ma.family_tree_id=$2 AND ma.person_id=r.entity_a_id JOIN family_tree_members mb ON mb.family_tree_id=$2 AND mb.person_id=r.entity_b_id JOIN family_trees ft ON ft.family_tree_id=$2 AND ft.project_id=r.project_id WHERE r.project_id=$1 AND r.entity_a_type='person' AND r.entity_b_type='person' ORDER BY r.relationship_id`:`SELECT r.entity_a_id::int AS a,r.entity_b_id::int AS b,rt.code,rt.label,rt.inverse_label AS "inverseLabel",rt.directed,rt.category,'relationship'::text AS source FROM relationships r JOIN relationship_types rt ON rt.relationship_type_id=r.relationship_type_id WHERE r.project_id=$1 AND r.entity_a_type='person' AND r.entity_b_type='person' ORDER BY r.relationship_id`;
  const legacySql=named?`SELECT p.parent_id AS a,p.child_id AS b,'parent'::text AS code,'Parent'::text AS label,'Child'::text AS "inverseLabel",true AS directed,'family'::text AS category,'legacy_parent_child'::text AS source FROM parent_child_relationships p JOIN npcs pa ON pa.n_id=p.parent_id JOIN npcs ch ON ch.n_id=p.child_id JOIN family_tree_members ma ON ma.family_tree_id=$2 AND ma.person_id=p.parent_id JOIN family_tree_members mb ON mb.family_tree_id=$2 AND mb.person_id=p.child_id JOIN family_trees ft ON ft.family_tree_id=$2 AND ft.project_id=pa.camp_id WHERE pa.camp_id=$1 AND ch.camp_id=$1`:`SELECT p.parent_id AS a,p.child_id AS b,'parent'::text AS code,'Parent'::text AS label,'Child'::text AS "inverseLabel",true AS directed,'family'::text AS category,'legacy_parent_child'::text AS source FROM parent_child_relationships p JOIN npcs pa ON pa.n_id=p.parent_id JOIN npcs ch ON ch.n_id=p.child_id WHERE pa.camp_id=$1 AND ch.camp_id=$1`;
