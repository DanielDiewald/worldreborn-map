...
export type FamilyTreeNode={personId:number;name:string;image:string;kind:"God"|"Player Character"|"NPC / Character";gender:string|null;title:string|null;roleLabel:string|null;branchLabel:string|null};
export type FamilyTreeEdge={a:number;b:number;code:string;label:string;inverseLabel:string|null;directed:boolean;category:string;source:string;metadata:Record<string,unknown>};
...
export async function getFamilyTreeGraph(projectId:number,treeRef:number|"all"){
  ...
  const peopleSql=named?`SELECT n.n_id AS "personId",n.name,n.image,CASE WHEN g.g_id IS NOT NULL THEN 'God' WHEN EXISTS(SELECT 1 FROM chars ch WHERE ch.n_id=n.n_id) THEN 'Player Character' ELSE 'NPC / Character' END AS kind,n.gender,n.title,m.role_label AS "roleLabel",m.branch_label AS "branchLabel" FROM family_tree_members m JOIN family_trees ft ON ft.family_tree_id=m.family_tree_id JOIN npcs n ON n.n_id=m.person_id AND n.camp_id=ft.project_id LEFT JOIN gods g ON g.n_id=n.n_id WHERE ft.project_id=$1 AND m.family_tree_id=$2 AND n.archived_at IS NULL ORDER BY m.sort_order,n.name,n.n_id`:`SELECT n.n_id AS "personId",n.name,n.image,CASE WHEN g.g_id IS NOT NULL THEN 'God' WHEN EXISTS(SELECT 1 FROM chars ch WHERE ch.n_id=n.n_id) THEN 'Player Character' ELSE 'NPC / Character' END AS kind,n.gender,n.title,NULL::varchar AS "roleLabel",NULL::varchar AS "branchLabel" FROM npcs n LEFT JOIN gods g ON g.n_id=n.n_id WHERE n.camp_id=$1 AND n.archived_at IS NULL ORDER BY n.name,n.n_id`;
  // The tree membership is still defined only by family relationships, but once two people are already in the
  // rendered tree every canonical person-to-person relationship between them is useful context and should be shown.
  const relationshipSql=named?`SELECT r.entity_a_id::int AS a,r.entity_b_id::int AS b,rt.code,rt.label,rt.inverse_label AS "inverseLabel",rt.directed,rt.category,'relationship'::text AS source,r.metadata FROM relationships r JOIN relationship_types rt ON rt.relationship_type_id=r.relationship_type_id JOIN family_tree_members ma ON ma.family_tree_id=$2 AND ma.person_id=r.entity_a_id JOIN family_tree_members mb ON mb.family_tree_id=$2 AND mb.person_id=r.entity_b_id JOIN family_trees ft ON ft.family_tree_id=$2 AND ft.project_id=r.project_id WHERE r.project_id=$1 AND r.entity_a_type='person' AND r.entity_b_type='person' ORDER BY r.relationship_id`:`SELECT r.entity_a_id::int AS a,r.entity_b_id::int AS b,rt.code,rt.label,rt.inverse_label AS "inverseLabel",rt.directed,rt.category,'relationship'::text AS source,r.metadata FROM relationships r JOIN relationship_types rt ON rt.relationship_type_id=r.relationship_type_id WHERE r.project_id=$1 AND r.entity_a_type='person' AND r.entity_b_type='person' ORDER BY r.relationship_id`;
  const legacySql=named?`SELECT p.parent_id AS a,p.child_id AS b,'parent'::text AS code,'Parent'::text AS label,'Child'::text AS "inverseLabel",true AS directed,'family'::text AS category,'legacy_parent_child'::text AS source,'{}'::jsonb AS metadata FROM parent_child_relationships p JOIN npcs pa ON pa.n_id=p.parent_id JOIN npcs ch ON ch.n_id=p.child_id JOIN family_tree_members ma ON ma.family_tree_id=$2 AND ma.person_id=p.parent_id JOIN family_tree_members mb ON mb.family_tree_id=$2 AND mb.person_id=p.child_id JOIN family_trees ft ON ft.family_tree_id=$2 AND ft.project_id=pa.camp_id WHERE pa.camp_id=$1 AND ch.camp_id=$1`:`SELECT p.parent_id AS a,p.child_id AS b,'parent'::text AS code,'Parent'::text AS label,'Child'::text AS "inverseLabel",true AS directed,'family'::text AS category,'legacy_parent_child'::text AS source,'{}'::jsonb AS metadata FROM parent_child_relationships p JOIN npcs pa ON pa.n_id=p.parent_id JOIN npcs ch ON ch.n_id=p.child_id WHERE pa.camp_id=$1 AND ch.camp_id=$1`;
