...
export async function tagEntity(args:{projectId:number;tagId:number;entityType:string;entityId:number}){entityType.parse(args.entityType);await pool.query(
`INSERT INTO entity_tags(project_id,tag_id,entity_type,entity_id)
 ...
 ON CONFLICT(project_id,tag_id,entity_type,entity_id) DO NOTHING`,[args.projectId,args.tagId,args.entityType,args.entityId]);}
...
export async function listEntityTags(args:{projectId:number;entityType:string;entityId:number}){const result=await pool.query<{tag_id:string;name:string}>(
