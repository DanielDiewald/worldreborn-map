...
export async function tagEntity(args:{projectId:number;tagId:number;entityType:string;entityId:number}){entityType.parse(args.entityType);await assertEntityBelongsToProject(args.projectId,args.entityType,args.entityId);const result=await pool.query(
`INSERT INTO entity_tags(project_id,tag_id,entity_type,entity_id)
 ...
 ON CONFLICT(project_id,tag_id,entity_type,entity_id) DO NOTHING`,[args.projectId,args.tagId,args.entityType,args.entityId]);if(result.rowCount===0){const tag=await pool.query("SELECT 1 FROM tags WHERE project_id=$1 AND tag_id=$2",[args.projectId,args.tagId]);if(tag.rowCount!==1)throw new Error("Tag does not belong to this project.");}}

export async function untagEntity(args:{projectId:number;tagId:number;entityType:string;entityId:number}){await pool.query(
`DELETE FROM entity_tags WHERE project_id=$1 AND tag_id=$2 AND entity_type=$3 AND entity_id=$4`,[args.projectId,args.tagId,args.entityType,args.entityId]);}

export async function listEntityTags(args:{projectId:number;entityType:string;entityId:number}){await assertEntityBelongsToProject(args.projectId,args.entityType,args.entityId);const result=await pool.query<{tag_id:string;name:string}>(
`SELECT t.tag_id,t.name FROM entity_tags et JOIN tags t ON t.tag_id=et.tag_id AND t.project_id=et.project_id
 WHERE et.project_id=$1 AND et.entity_type=$2 AND et.entity_id=$3 ORDER BY t.name`,[args.projectId,args.entityType,args.entityId]);return result.rows;}
