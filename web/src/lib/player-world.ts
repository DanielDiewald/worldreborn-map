...
`SELECT g.g_id AS id,COALESCE(v.name_override,n.name) AS name,g.title,g.domain,g.faction,COALESCE(v.description_override,n.public_description) AS description,COALESCE(v.image_override,n.image) AS image
 FROM gods g JOIN npcs n ON n.n_id=g.n_id
 LEFT JOIN entity_visibility ev ON ev.project_id=n.camp_id AND ev.player_id=$2 AND ev.entity_type='god' AND ev.entity_id=g.g_id
 LEFT JOIN player_entity_variants v ON v.project_id=n.camp_id AND v.player_id=$2 AND v.entity_type='god' AND v.entity_id=g.g_id AND v.mode='override'
...
`SELECT l.loc_id AS id,l.name,l.location_type,l.description,l.parent_loc_id AS parent_id,p.name AS parent_name,l.population
 ...
 WHERE l.camp_id=$1 AND l.archived_at IS NULL AND COALESCE(ev.visible,l.visibility_mode='all_players')
 AND (p.loc_id IS NULL OR COALESCE(pv.visible,p.visibility_mode='all_players')) ORDER BY l.name`,[projectId,playerId]);return r.rows;}

export async function listVisibleGroups(projectId:number,playerId:number){const r=await pool.query<{id:number;name:string;group_type:string|null;notes:string;motto:string;image:string;location_name:string|null;members:number}>(
`SELECT g.gr_id AS id,g.name,g.group_type,g.notes,g.motto,g.image,l.name AS location_name,
 (SELECT count(*)::int FROM group_memberships gm WHERE gm.project_id=g.camp_id AND gm.group_id=g.gr_id AND NOT EXISTS(SELECT 1 FROM entity_visibility mev WHERE mev.project_id=gm.project_id AND mev.player_id=$2 AND mev.entity_type=gm.entity_type AND mev.entity_id=gm.entity_id AND mev.visible=false)) AS members
 FROM groups g
 ...
 WHERE g.camp_id=$1 AND g.archived_at IS NULL AND COALESCE(ev.visible,g.visibility_mode='all_players')
 AND COALESCE(lv.visible,l.visibility_mode='all_players') ORDER BY g.name`,[projectId,playerId]);return r.rows;}
...
`SELECT c.char_id,n.n_id,n.name,n.image,c.race,c.class,c.age,c.alive,n.public_description,l.name AS location_name
 FROM chars a JOIN npcs n ON n.n_id=a.n_id JOIN charakters c ON c.n_id=n.n_id JOIN locations l ON l.loc_id=c.loc_id AND l.camp_id=n.camp_id
 WHERE a.user_id=$2 AND n.camp_id=$1 AND n.archived_at IS NULL LIMIT 1`,[projectId,playerId]);return r.rows[0]??null;}
