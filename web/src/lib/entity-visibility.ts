...
export async function listPlayerEntityAccess(args: { projectId: number; entityType: string; entityId: number; baseVisibility: string }) {
  const ref = await assertEntityBelongsToProject(args.projectId, args.entityType, args.entityId);
  const result = await pool.query<{player_id:number;player_name:string;explicit_visible:boolean|null;variant_name:string|null;variant_description:string|null;variant_id:string|null}>(
    `SELECT u.user_id AS player_id,COALESCE(u.display_name,u.name) AS player_name,ev.visible AS explicit_visible,v.name_override AS variant_name,v.description_override AS variant_description,v.variant_id
      ...
      WHERE u.camp_id=$1 ORDER BY COALESCE(u.display_name,u.name),u.user_id`,[args.projectId,ref.type,ref.id]);
...
export async function setPlayerEntityVisibility(args:{projectId:number;playerId:number;entityType:string;entityId:number;visible:boolean}){
  ...
  await pool.query(`INSERT INTO entity_visibility(project_id,player_id,entity_type,entity_id,visible) VALUES($1,$2,$3,$4,$5) ON CONFLICT(project_id,player_id,entity_type,entity_id) DO UPDATE SET visible=EXCLUDED.visible,updated_at=now()`,[args.projectId,args.playerId,ref.type,ref.id,args.visible]);
  await pool.query(`INSERT INTO audit_log(project_id,actor_type,action,entity_type,entity_id,metadata) VALUES($1,'admin','visibility.changed',$2,$3,$4::jsonb)`,[args.projectId,ref.type,ref.id,JSON.stringify({player_id:args.playerId,visible:args.visible,source_type:args.entityType,source_id:args.entityId})]);
}
...
export async function upsertPlayerVariant(args:{projectId:number;playerId:number;entityType:string;entityId:number;input:unknown}){
  const input=variantSchema.parse(args.input);const ref=await assertEntityBelongsToProject(args.projectId,args.entityType,args.entityId);const player=await pool.query("SELECT 1 FROM users WHERE camp_id=$1 AND user_id=$2",[args.projectId,args.playerId]);if(player.rowCount!==1)throw new Error("Player does not belong to this project.");
  await pool.query(`INSERT INTO player_entity_variants(project_id,player_id,entity_type,entity_id,mode,name_override,description_override,image_override) VALUES($1,$2,$3,$4,'override',$5,$6,$7) ON CONFLICT(project_id,player_id,entity_type,entity_id) WHERE mode='override' DO UPDATE SET name_override=EXCLUDED.name_override,description_override=EXCLUDED.description_override,image_override=EXCLUDED.image_override,updated_at=now()`,[args.projectId,args.playerId,ref.type,ref.id,input.name||null,input.description||null,input.image||null]);
  await setPlayerEntityVisibility({projectId:args.projectId,playerId:args.playerId,entityType:ref.type,entityId:ref.id,visible:true});
  await pool.query(`INSERT INTO audit_log(project_id,actor_type,action,entity_type,entity_id,metadata) VALUES($1,'admin','variant.upserted',$2,$3,$4::jsonb)`,[args.projectId,ref.type,ref.id,JSON.stringify({player_id:args.playerId,source_type:args.entityType,source_id:args.entityId})]);
}

export async function deletePlayerVariant(args:{projectId:number;playerId:number;entityType:string;entityId:number}){
  const ref=await assertEntityBelongsToProject(args.projectId,args.entityType,args.entityId);
  await pool.query(`DELETE FROM player_entity_variants WHERE project_id=$1 AND player_id=$2 AND entity_type=$3 AND entity_id=$4 AND mode='override'`,[args.projectId,args.playerId,ref.type,ref.id]);
}
