import type { JsonMapGeometry, LandMaskGuide, MapCoordinate, MapExtent } from "./map-geometry-guides";
import { maskPixelToMapCoordinate } from "./map-geometry-guides";
...
function mapToPixel(guide: LandMaskGuide, coordinate: MapCoordinate): MapCoordinate {
  ...
  return [
    ((coordinate[0] - minX) / Math.max(1e-9, maxX - minX)) * (guide.width - 1),
    ((maxY - coordinate[1]) / Math.max(1e-9, maxY - minY)) * (guide.height - 1),
  ];
}
function toPixelPolygons(geometry: JsonMapGeometry, guide: LandMaskGuide) {
  return geometryPolygons(geometry).map((polygon) => polygon.map((ring) => ring.map((coordinate) => mapToPixel(guide, coordinate))));
}
...
function intersectMasks(a: Uint8Array, b: Uint8Array) {
  ...
  for (let index = 0; index < result.length; index += 1) result[index] = a[index] && b[index] ? 1 : 0;
  return result;
}
...
function traceRings(edges: Edge[]) {
  ...
  for (let seed = 0; seed < edges.length; seed += 1) {
    ...
    const startEdge = edges[seed], startKey = key(startEdge.start);
    let currentIndex = seed, guard = 0;
    ...
    while (guard++ <= edges.length + 4) {
      ...
      used[currentIndex] = 1; ring.push([edge.end[0] / 2, edge.end[1] / 2]);
...
function geometryFromMask(original: JsonMapGeometry, guide: LandMaskGuide, mask: Uint8Array) {
  const grouped = groupRings(traceRings(boundaryEdges(mask, guide.width, guide.height)));
  if (!grouped.length) return original;
  const polygons = grouped.map((polygon) => polygon.map((ring) => convertRingToMap(ring, guide)));
  if (polygons.length === 1) return { ...original, type: "Polygon", coordinates: polygons[0] };
  return { ...original, type: "MultiPolygon", coordinates: polygons };
}

/** True raster intersection: hand-drawn territory ∩ Rock-3 land pixels. */
export function clipPolygonToLandMask(geometry: JsonMapGeometry, guide: LandMaskGuide): JsonMapGeometry {
  const source = toPixelPolygons(geometry, guide);
  if (!source.length) return geometry;
...
function localGuideForParent(parent: JsonMapGeometry, maxSide: number) {
  const extent = geometryExtent(parent);
  if (!extent) return null;
  const rawWidth = Math.max(1e-6, extent[2] - extent[0]), rawHeight = Math.max(1e-6, extent[3] - extent[1]);
  const padX = rawWidth * 0.002 + 1e-6, padY = rawHeight * 0.002 + 1e-6;
  const padded: MapExtent = [extent[0] - padX, extent[1] - padY, extent[2] + padX, extent[3] + padY];
  const aspect = rawWidth / rawHeight;
  const width = aspect >= 1 ? maxSide : Math.max(96, Math.round(maxSide * aspect));
  const height = aspect >= 1 ? Math.max(96, Math.round(maxSide / aspect)) : maxSide;
  const guide: LandMaskGuide = { width, height, extent: padded, land: new Uint8Array(width * height), boundaryPixels: [] };
  guide.land = rasterizePolygons(toPixelPolygons(parent, guide), width, height);
  return guide;
}

/**
 * Boolean child ∩ parent clipping for provinces and regions.
 * A local high-resolution grid makes the shared outside border follow the parent polygon
 * instead of merely moving individual outside vertices to whichever edge is nearest.
 */
export function clipPolygonToParentMask(child: JsonMapGeometry, parent: JsonMapGeometry, maxSide = 1024): JsonMapGeometry {
  ...
  const guide = localGuideForParent(parent, Math.max(256, Math.min(1536, maxSide)));
  if (!guide) return child;
