...
function cornerOceanLuminance(data: Uint8ClampedArray, width: number, height: number) {
  ...
  for (const [startX, startY] of corners) {
    for (let y = startY; y < startY + sampleSize; y += 1) {
      for (let x = startX; x < startX + sampleSize; x += 1) values.push(luminance(data, y * width + x));
    }
  }
...
export function buildLandMaskGuide(data: Uint8ClampedArray, width: number, height: number, extent: MapExtent): LandMaskGuide {
  ...
  for (let y = 0; y < height; y += sampleStep) {
    for (let x = 0; x < width; x += sampleStep) sampled.push(luminance(data, y * width + x));
  }

  const low = percentile(sampled, 0.08);
  const high = percentile(sampled, 0.92);
  const threshold = (low + high) / 2;
  const oceanLum = cornerOceanLuminance(data, width, height);
  const landIsBright = Math.abs(oceanLum - low) <= Math.abs(oceanLum - high);
  ...
  for (let index = 0; index < width * height; index += 1) {
    ...
    land[index] = landIsBright ? Number(value > threshold) : Number(value < threshold);
  }
  ...
  for (let y = 1; y < height - 1; y += 1) {
    for (let x = 1; x < width - 1; x += 1) {
      ...
      if (
        land[y * width + x - 1] !== current ||
        land[y * width + x + 1] !== current ||
        land[(y - 1) * width + x] !== current ||
        land[(y + 1) * width + x] !== current
...
export function mapCoordinateToMaskPixel(guide: LandMaskGuide, coordinate: MapCoordinate): MapCoordinate {
  ...
  // ImageStatic maps the top row of the source image to maxY.
...
export function maskPixelToMapCoordinate(guide: LandMaskGuide, pixel: MapCoordinate): MapCoordinate {
  const [minX, minY, maxX, maxY] = guide.extent;
  const x = minX + (pixel[0] / Math.max(1, guide.width - 1)) * (maxX - minX);
  const y = maxY - (pixel[1] / Math.max(1, guide.height - 1)) * (maxY - minY);
  return [x, y];
}

function maskPixelIsLand(guide: LandMaskGuide, pixel: MapCoordinate) {
  const x = clamp(Math.round(pixel[0]), 0, guide.width - 1);
  const y = clamp(Math.round(pixel[1]), 0, guide.height - 1);
...
function nearestBoundaryPixel(guide: LandMaskGuide, pixel: MapCoordinate) {
  ...
  for (const candidate of guide.boundaryPixels) {
    const dx = candidate[0] - pixel[0];
    const dy = candidate[1] - pixel[1];
...
function sameCoordinate(a: MapCoordinate, b: MapCoordinate, epsilon = 1e-6) {
  return Math.abs(a[0] - b[0]) <= epsilon && Math.abs(a[1] - b[1]) <= epsilon;
}

function closeRing(points: MapCoordinate[]) {
  if (!points.length) return points;
  if (!sameCoordinate(points[0], points[points.length - 1])) points.push([...points[0]] as MapCoordinate);
  return points;
}

function dedupeRing(points: MapCoordinate[]) {
  const result: MapCoordinate[] = [];
  for (const point of points) {
    if (!result.length || !sameCoordinate(result[result.length - 1], point)) result.push(point);
  }
...
function polygonFromUnknown(value: unknown): MapCoordinate[][] | null {
  if (!Array.isArray(value)) return null;
  const rings = value.map(ringFromUnknown);
  return rings.every(Boolean) ? rings as MapCoordinate[][] : null;
}
...
function densifyRingInMaskPixels(ring: MapCoordinate[], guide: LandMaskGuide, maxStepPixels: number) {
  ...
  for (let index = 0; index < ring.length - 1; index += 1) {
    ...
    const a = mapCoordinateToMaskPixel(guide, start);
    const b = mapCoordinateToMaskPixel(guide, end);
    const length = Math.hypot(b[0] - a[0], b[1] - a[1]);
    const steps = Math.max(1, Math.ceil(length / Math.max(1, maxStepPixels)));
...
function conformRingToLandMask(ring: MapCoordinate[], guide: LandMaskGuide, maxStepPixels: number) {
  ...
  const conformed = dense.map((coordinate) => {
    const pixel = mapCoordinateToMaskPixel(guide, coordinate);
    if (maskPixelIsLand(guide, pixel)) return coordinate;
    const boundary = nearestBoundaryPixel(guide, pixel);
    return boundary ? maskPixelToMapCoordinate(guide, boundary) : coordinate;
  });
  return dedupeRing(conformed);
}
...
export function conformPolygonToLandMask(geometry: JsonMapGeometry, guide: LandMaskGuide, maxStepPixels = 6): JsonMapGeometry {
  if (geometry.type === "Polygon") {
    const polygon = polygonFromUnknown(geometry.coordinates);
    if (!polygon || !guide.boundaryPixels.length) return geometry;
    return { ...geometry, coordinates: polygon.map((ring, index) => index === 0 ? conformRingToLandMask(ring, guide, maxStepPixels) : ring) };
  }
  if (geometry.type === "MultiPolygon" && Array.isArray(geometry.coordinates)) {
    const polygons = geometry.coordinates.map(polygonFromUnknown);
    if (!polygons.every(Boolean) || !guide.boundaryPixels.length) return geometry;
    return {
      ...geometry,
      coordinates: (polygons as MapCoordinate[][][]).map((polygon) => polygon.map((ring, index) => index === 0 ? conformRingToLandMask(ring, guide, maxStepPixels) : ring)),
    };
  }
  return geometry;
}
...
export function geometryContainsCoordinate(geometry: JsonMapGeometry, point: MapCoordinate) {
  ...
  if (geometry.type === "MultiPolygon" && Array.isArray(geometry.coordinates)) {
    return geometry.coordinates.some((value) => {
      const polygon = polygonFromUnknown(value);
      return polygon ? pointInPolygon(point, polygon) : false;
    });
  }
  return false;
...
function closestPointOnSegment(point: MapCoordinate, start: MapCoordinate, end: MapCoordinate): MapCoordinate {
  ...
  const ratio = clamp(((point[0] - start[0]) * dx + (point[1] - start[1]) * dy) / denominator, 0, 1);
...
export function closestPointOnPolygonGeometry(geometry: JsonMapGeometry, point: MapCoordinate) {
  let best: MapCoordinate | null = null;
  let bestDistance = Number.POSITIVE_INFINITY;
  for (const ring of polygonBoundaryRings(geometry)) {
    for (let index = 0; index < ring.length - 1; index += 1) {
      const candidate = closestPointOnSegment(point, ring[index], ring[index + 1]);
...
function densifyRing(ring: MapCoordinate[], maxStep: number) {
  const result: MapCoordinate[] = [];
  ...
  for (let index = 0; index < closed.length - 1; index += 1) {
    ...
    const steps = Math.max(1, Math.ceil(length / Math.max(1e-6, maxStep)));
    for (let step = 0; step < steps; step += 1) {
      const ratio = step / steps;
      result.push([start[0] + (end[0] - start[0]) * ratio, start[1] + (end[1] - start[1]) * ratio]);
    }
  }
  result.push(closed[closed.length - 1]);
  return result;
}

function conformRingToParent(ring: MapCoordinate[], parent: JsonMapGeometry, maxStep: number) {
  const dense = densifyRing(ring, maxStep);
  return dedupeRing(dense.map((point) => {
    if (geometryContainsCoordinate(parent, point)) return point;
    return closestPointOnPolygonGeometry(parent, point) ?? point;
  }));
}

export function conformPolygonToParent(geometry: JsonMapGeometry, parent: JsonMapGeometry, maxStep: number): JsonMapGeometry {
  if (!["Polygon", "MultiPolygon"].includes(parent.type)) return geometry;
  if (geometry.type === "Polygon") {
    const polygon = polygonFromUnknown(geometry.coordinates);
    if (!polygon) return geometry;
    return { ...geometry, coordinates: polygon.map((ring, index) => index === 0 ? conformRingToParent(ring, parent, maxStep) : ring) };
  }
  if (geometry.type === "MultiPolygon" && Array.isArray(geometry.coordinates)) {
    const polygons = geometry.coordinates.map(polygonFromUnknown);
    if (!polygons.every(Boolean)) return geometry;
    return {
      ...geometry,
      coordinates: (polygons as MapCoordinate[][][]).map((polygon) => polygon.map((ring, index) => index === 0 ? conformRingToParent(ring, parent, maxStep) : ring)),
    };
  }
  return geometry;
}
