...
export function ImageSourceInput({ current, pathName = "image", fileName = "imageFile", label = "Bild / Porträt" }: Props) {
  return <fieldset className="image-source-fieldset">
    <legend>{label}</legend>
    {validImage(current) ? <div className="image-source-preview">
      <EntityImageFrame className="image-source-square-preview" src={current} alt="Aktuelles Bild"/>
      <small>Aktuelle Quelle: <code>{current}</code></small>
    ...
    <p className="section-help">Der 1:1-Rahmen schneidet das Original nicht zu: Hoch- und Querformat bleiben vollständig sichtbar. Ein ausgewählter Upload hat Vorrang vor dem Pfad.</p>
  </fieldset>;
}
