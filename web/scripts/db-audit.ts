...
async function main(){
  ...
  try{
    ...
    warning("person.legacy_gender_value",await scalar("SELECT count(*) value FROM npcs WHERE gender IS NULL OR gender NOT IN ('male','female','hermaphrodite')"),"Legacy free-text gender values are preserved until the person is edited and explicitly assigned a canonical gender.");
