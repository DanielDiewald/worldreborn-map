...
import { clipPolygonToLandMask } from "../src/components/map/map-raster-clip";
...
test("country clipping removes water around a rectangular land mass", () => {
  ...
  const mask = guide(8, 6, cells);
  const clipped = clipPolygonToLandMask({
    type: "Polygon",
    coordinates: [[[0, 0], [7, 0], [7, 5], [0, 5], [0, 0]]],
  }, mask);
...
test("country clipping keeps disconnected islands as a MultiPolygon", () => {
  const mask = guide(10, 6, [
    [1, 2], [2, 2], [1, 3], [2, 3],
    [7, 1], [8, 1], [7, 2], [8, 2],
  ]);
  const clipped = clipPolygonToLandMask({
    type: "Polygon",
    coordinates: [[[0, 0], [9, 0], [9, 5], [0, 5], [0, 0]]],
  }, mask);
