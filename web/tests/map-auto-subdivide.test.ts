...
test("automatic subdivision creates exactly the requested number of parts", () => {
  const result = autoSubdividePolygon(rectangle, { count: 6, seed: 42, irregularity: 0.7, balance: 0.9, maxSide: 320 });
  assert.ok(result);
...
test("automatic subdivision is deterministic for the same seed and settings", () => {
  const first = autoSubdividePolygon(rectangle, { count: 5, seed: 12345, irregularity: 0.7, balance: 0.8, maxSide: 300 });
  const second = autoSubdividePolygon(rectangle, { count: 5, seed: 12345, irregularity: 0.7, balance: 0.8, maxSide: 300 });
...
test("border organicity changes the internal partition without changing the requested count", () => {
  ...
  const organic = autoSubdividePolygon(rectangle, { count: 6, seed: 991, irregularity: 0.95, balance: 0.82, maxSide: 300 });
