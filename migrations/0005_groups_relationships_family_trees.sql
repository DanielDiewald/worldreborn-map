...
COMMENT ON COLUMN public.groups.members IS 'Estimated/total organization size. This is NOT the exact count of known relational memberships.';

CREATE OR REPLACE FUNCTION public.check_group_parent_project()
...
CREATE OR REPLACE FUNCTION public.sync_is_part_of_to_group_memberships()
RETURNS trigger LANGUAGE plpgsql AS $$
...
CREATE OR REPLACE FUNCTION public.upsert_person_group_membership(
...
CREATE OR REPLACE FUNCTION public.delete_person_group_membership(p_project_id integer,p_group_id integer,p_person_id integer)
...
CREATE OR REPLACE FUNCTION public.check_family_tree_root_project()
RETURNS trigger LANGUAGE plpgsql AS $$
...
CREATE OR REPLACE FUNCTION public.check_family_tree_member_project()
RETURNS trigger LANGUAGE plpgsql AS $$
