...
-- IF NOT EXISTS / DROP IF EXISTS intentionally make this branch migration safe to
-- retry after manual SQL experiments or an interrupted development migration.
ALTER TABLE public.project_map_layers
  ADD COLUMN IF NOT EXISTS media_id bigint REFERENCES public.media(media_id) ON UPDATE CASCADE ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS layer_role character varying(80),
...
CREATE INDEX IF NOT EXISTS project_map_layers_media_idx ON public.project_map_layers(project_id, media_id) WHERE media_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS project_map_layers_role_idx ON public.project_map_layers(project_id, map_id, layer_role) WHERE layer_role IS NOT NULL;

-- Locations become a reusable spatial hierarchy. Existing location_type remains for free-form lore labels.
ALTER TABLE public.locations
  ADD COLUMN IF NOT EXISTS location_kind character varying(40) NOT NULL DEFAULT 'other',
  ADD COLUMN IF NOT EXISTS slug character varying(140),
  ADD COLUMN IF NOT EXISTS map_id bigint REFERENCES public.project_maps(map_id) ON UPDATE CASCADE ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS map_feature_id bigint REFERENCES public.map_features(feature_id) ON UPDATE CASCADE ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS capital_loc_id integer REFERENCES public.locations(loc_id) ON UPDATE CASCADE ON DELETE SET NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS locations_project_slug_unique ON public.locations(camp_id, slug) WHERE slug IS NOT NULL AND archived_at IS NULL;
CREATE INDEX IF NOT EXISTS locations_hierarchy_idx ON public.locations(camp_id, parent_loc_id, location_kind, loc_id) WHERE archived_at IS NULL;
CREATE INDEX IF NOT EXISTS locations_map_feature_idx ON public.locations(camp_id, map_id, map_feature_id) WHERE map_id IS NOT NULL OR map_feature_id IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS public.person_location_assignments (
...
CREATE INDEX IF NOT EXISTS person_location_person_idx ON public.person_location_assignments(project_id, person_id, role, is_primary DESC);
CREATE INDEX IF NOT EXISTS person_location_location_idx ON public.person_location_assignments(project_id, location_id, role);
CREATE UNIQUE INDEX IF NOT EXISTS person_location_primary_role_unique ON public.person_location_assignments(project_id, person_id, role) WHERE is_primary;
